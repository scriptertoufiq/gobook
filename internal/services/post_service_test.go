...
// Updating writes the new version through, so the next read is a cache hit
// carrying the edit — never the pre-edit copy, and never a database round trip.
func TestUpdateWritesTheNewVersionIntoTheCache(t *testing.T) {
	...
	if source != cache.FromCache {
		t.Errorf("the read after an edit should be served from cache, got %q", source)
	}
	if repo.findCalls != before {
